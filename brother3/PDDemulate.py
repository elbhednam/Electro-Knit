...
# the knitting machine on the windows file system. 
#
...
class DiskSector():
    def __init__(self, fn):
        self.sectorSz = 1024 # Logical sector size
        self.idSz = 12
        self.data = ''
        self.id = ''
  
        dfn = fn + ".dat" # Create nn.dat for sector nn
        idfn = fn + ".id" # Create nn.id for sector nn

        try:
            try:
                self.df = open(dfn, 'r+b') # Open .dat file for reading and writing
            except IOError:
                self.df = open(dfn, 'wb') 

            try:
                self.idf = open(idfn, 'r+b') # Open .id file for reading and writing
            except IOError:
        ...
        try:
            if dfs == 0:
                # New or empty file
                self.data = b''.join([b'\x00' for num in range(self.sectorSz)]) #'\x00' is the null character
                self.writeDFile()
        ...
        try:
            if idfs == 0:
                # New or empty file
                self.id = b''.join([b'\x00' for num in range(self.idSz)])
                self.writeIdFile()
            elif idfs == self.idSz:
                # Existing file
    ...
    def format(self):
        self.data = b''.join([b'\x00' for num in range(self.sectorSz)])
        self.writeDFile()
        self.id = b''.join([b'\x00' for num in range(self.idSz)])
        self.writeIdFile()

    def writeDFile(self):
        self.df.seek(0) # Sets the file’s current position.
        self.df.write(self.data)
        self.df.flush() # Flush the internal buffer
        return

    def writeIdFile(self):
        self.idf.seek(0) # Sets the file’s current position.
        self.idf.write(self.id)
        self.idf.flush() # Flush the internal buffer
        return
    ...
    def dumpId(self):
        for i in self.id:
			...
			# '%02X ' pattern specifies a two char output with prepended 0 if its only 1 character in length
            print('%02d ' % i, end=' ')
        print()
...
class Disk():
    ...
    def __init__(self, basename):
        ...
        for i in range(self.numSectors):
            ...
            ds = DiskSector(fname) # Create disk sector object
            self.Sectors.append(ds) # Add new disk sector object to sector array
        return
    ...
    def format(self):
        for i in range(self.numSectors):
            self.Sectors[i].format() # Call DiskSector's format() method for each DiskSector object
        return
...
class PDDemulator():
    ...
    def __init__(self, basename): # basename = img
    ...
    def open(self, cport='COM3'): # Default value of COM3 for my usb-to-serial cable
        if self.noserial is False:
		    # Changed timeout from 1 second to none
            self.ser = serial.Serial(port=cport, baudrate=9600, parity='N', stopbits=1, timeout=None, xonxoff=0, rtscts=0, dsrdtr=0)
            if self.ser == None:
    ...
    def readFDDRequest(self):
        ...
        while 1:
            inc = self.readchar()
            if inc == b'\r': # ser.read returns bytes
                break
            elif inc == b' ': # ser.read returns bytes
                continue
            else:
                inbuf.append(inc)
    ...
    def handleFDCmodeRequest(self, cmd):
        ...
        # LSN defaults rto 1 if not supplied
        #
        ...
        # Preamble for an opmode request is always 'ZZ'
        if cmd == b'Z':
        ...
        # Switches the disk drive between FDC-emulation mode and Operation mode
		# Not used by the brother knitting machine
        elif cmd == b'M':
            print('FDC Change Modes')
            raise
            # following parameter - 0=FDC, 1=Operating

		# Check device condition - Sends the condition of the drive in the third and fourth bytes of the result code
		# Not used by brother knitting machine
        elif cmd == b'D':
            print('FDC Check Device')
            raise
            # Sends result in third and fourth bytes of result code
            # See PDD software manual for meaning of result code bits

		# F - Format with verify
		# G - Format without verify
        elif cmd == b'F'or cmd == b'G':
		...
		# A - Read the ID section of the specified physical sector
...
if __name__ == "__main__":
    ...
    emu = PDDemulator(sys.argv[1]) # img

    emu.open(cport=sys.argv[2]) # COM3
