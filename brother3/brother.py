...
def nibbles(achar):
    msn = (achar & 0xF0) >> 4 # achar AND 1111 0000 shifted right 4 places gives most significant nibble
    lsn = achar & 0x0F # achar AND 0000 1111 gives least significant nibble
    return msn, lsn
...
class brotherFile(object):
    ...
    def setIndexedByte(self, index, b):
        ...
        # and convert back
        dataarray = array('B') # changed from 'c' to 'B' for unsigned char
    ...
    def getRowData(self, pattOffset, stitches, rownumber):
        ...
        for i in range(int(startnib), int(endnib), 1):
    ...
    def getPatterns(self, patternNumber = None):
        ...
        for pi in range(1, 100):
            ...
            idx = idx + 1
            unknown = self.data[idx]
            idx = idx + 1
            rh, rt = nibbles(self.data[idx])# row: hundreds, row: tens
            idx = idx + 1
            ro, sh = nibbles(self.data[idx])# row: ones, stitches: hundreds
            idx = idx + 1
            st, so = nibbles(self.data[idx])# stitches: tens, stitches: ones
            idx = idx + 1
            unk, ph = nibbles(self.data[idx])# unknown, pattern: hundreds
            idx = idx + 1
            pt, po = nibbles(self.data[idx])# pattern: tens, pattern: ones
            idx = idx + 1
            rows = hto(rh,rt,ro)
    ...
    def getPatternMemo(self, patternNumber):
        ...
        # memo is padded to an even byte
